...
export const launchYear = () => async (dispatch) => {
  try {
    ...
    if (res !== undefined && res !== null) {
      const years =
        res && Array.prototype.map.call(res, (item) => item.launch_year);
      const cardDetails =
...
export const launchLandSuccess = (type) => async (dispatch) => {
  try {
    ...
    if (type === "landSuccess")
      fetchUrl = `${baseUrl}&launch_success=true&land_success=true`;
    if (type === "launchfail")
      fetchUrl = `${baseUrl}&launch_success=true&land_success=fail`;
